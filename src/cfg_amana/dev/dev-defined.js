...
g.medical_data_fixers = {
    ...
    diagnosis: function (value) {
        ...
        // Look for the first range that matches any code.
        for (var i = 0; i < g.diagnoses.ranges.length; i++) {
            var entry = g.diagnoses.ranges[i];
            var bounds = entry[0].split('-');
            var code_min = bounds[0];
            var code_max = bounds[bounds.length - 1];
            var description = entry[1];
            for (var j = 0; j < codes.length; j++) {
                if (code_min <= codes[j] && codes[j] <= code_max) {
...
g.medical_data_filter = function(record) {
    // Show only infectious, nutritional, metabolic, and respiratory diseases.
    return (record['disease_list'] || '').match(/^[ABEJ]/);
};
